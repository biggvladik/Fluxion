...
pub struct GasApi {
    ...
    pub get_spectr: Symbol<'static, unsafe extern "system" fn(
        *mut c_float,
        *mut c_float,
        *mut c_float,
    )>,
...
impl GasApi {
    pub fn new(path: &str) -> Result<Self, Box<dyn std::error::Error>> {
        ...
        unsafe {
            ...
            let start: Symbol<unsafe extern "system" fn(*mut c_int) -> c_int> =
                lib.get(b"Start")?;

            let start = transmute(start);

            let init: Symbol<unsafe extern "system" fn(*mut c_int) -> c_int> =
                lib.get(b"Init")?;
            let init = transmute(init);

            let get_error: Symbol<unsafe extern "system" fn(*mut c_char)> =
                lib.get(b"GetError")?;
            let get_error = transmute(get_error);

            let get_warning: Symbol<unsafe extern "system" fn(*mut c_char)> =
                lib.get(b"GetWarning")?;
            let get_warning = transmute(get_warning);

            let get_size: Symbol<unsafe extern "system" fn() -> c_int> =
                lib.get(b"GetSize")?;
            let get_size = transmute(get_size);

            let get_spectr: Symbol<unsafe extern "system" fn(
                *mut c_float,
                *mut c_float,
                *mut c_float,
            )> = lib.get(b"GetSpectr")?;
